...
async function main() {
  ...
  console.log(
    `Found ${appsWithHistory.size} applications that already have history.`,
  );

  const appsToBackfill = apps.filter((app) => !appsWithHistory.has(app.id));
  console.log(
    `Need to backfill history for ${appsToBackfill.length} applications.`,
  );

  if (appsToBackfill.length === 0) {
    console.log(
      "All applications already have history records. Nothing to do.",
    );
    return;
  }
  ...
  console.log(
    `Successfully backfilled ${valuesToInsert.length} status history records!`,
  );
}
