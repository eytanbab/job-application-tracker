  ...
  return (
    <Card className="w-full bg-card shadow-2xs border border-border/30 rounded-xl hover:shadow-xs transition-shadow">
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        <CardTitle className="text-base font-bold">Role Targeting Distribution</CardTitle>
        <Briefcase className="h-4 w-4 text-primary/80" />
      </CardHeader>
      <CardContent className="flex flex-col gap-3.5 pt-2">
        {roles.length === 0 ? (
          <p className="text-sm text-muted-foreground py-4 text-center">No role targeting data available</p>
        ) : (
          <>
                ...
                <div key={item.name} className="flex flex-col gap-1.5">
                  <div className="flex items-center justify-between text-xs font-medium">
                    ...
                    <span className="text-muted-foreground font-semibold text-[11px]">
                      {item.count} {item.count === 1 ? 'app' : 'apps'} ({pct.toFixed(1)}%)
                    </span>
                  </div>
