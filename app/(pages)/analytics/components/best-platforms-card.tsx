...
type Props = {
  bestPlatform: { name: string; total: number; interviews: number; interviewRate: number } | null;
  secondBest: { name: string; total: number; interviews: number; interviewRate: number } | null;
  multiplier: number;
};

export function BestPlatformsCard({ bestPlatform, secondBest, multiplier }: Props) {
  return (
    <Card className="bg-card shadow-2xs border border-border/30 rounded-xl hover:shadow-xs transition-shadow">
      ...
      <CardContent>
        {!bestPlatform ? (
          <div className="pt-2">
            <p className="text-sm text-muted-foreground">Apply to more jobs to see platform insights</p>
          </div>
        ) : (
          <div className="flex flex-col gap-1">
            ...
            {multiplier > 1 && secondBest ? (
              <p className="text-xs text-muted-foreground mt-1">
                You get {multiplier.toFixed(1)}× more interviews from {bestPlatform.name} than {secondBest.name}. Focus there.
              </p>
            ) : (
              <p className="text-xs text-muted-foreground mt-1">
