      ...
      <section className="space-y-3">
        ...
        <Card className="w-full bg-card shadow-2xs border border-border/30 rounded-xl">
          <CardHeader className="pb-2">
            <CardTitle className="text-base font-bold">Platform Interview Yield Leaderboard</CardTitle>
            <CardDescription className="text-xs text-muted-foreground">
              Highest interview conversion rate platforms for the selected timeframe
            </CardDescription>
          </CardHeader>
          <CardContent className="pt-2">
            {platformRoi.length === 0 ? (
              <p className="text-sm text-muted-foreground py-4 text-center">No platform ROI data available</p>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {platformRoi.map((item, idx) => (
                  <div key={item.name} className="p-3 rounded-lg bg-background border border-border/40 flex flex-col justify-between gap-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {item.interviews} {item.interviews === 1 ? 'interview' : 'interviews'} from {item.total} total {item.total === 1 ? 'application' : 'applications'}
                    </p>
                  </div>
                ))}
              </div>
